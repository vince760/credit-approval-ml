...
DATA_PATH = os.path.join(REPO_ROOT, "data", "Credit_Card_Applications.csv")
...
def main():
    ...
    candidate_targets = [c for c in df.columns if str(c).strip().lower() in {"approved", "target", "class", "label", "y"}]
    ...
    # Model (strong default for tabular classification)
    model = XGBClassifier(
        n_estimators=400,
        learning_rate=0.05,
        max_depth=5,
